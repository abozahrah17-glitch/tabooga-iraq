...
(function() {
    try {
        ...
        // Clean up any stray mojibake if it exists
        document.querySelectorAll('div').forEach(p => {
            if (p.innerHTML && p.innerHTML.includes('Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª')) p.remove();
        });

        // Hook openAdminPanel to make sure UI is injected
        if (window.openAdminPanel && !window._hookedOpen7) {
            window._hookedOpen7 = true;
        ...
        if (window.renderAdminData && !window._paymentHooked7) {
            window._paymentHooked7 = true;
