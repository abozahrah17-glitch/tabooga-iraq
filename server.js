...
app.get('/tabooga.apk', (req, res) => {
    const candidates = [
        ...
        path.join(process.cwd(), 'app_stable/tabooga.apk'),
        path.join(process.cwd(), 'clean_code/tabooga.apk'),
        path.join(process.cwd(), 'clean_code/app_stable/tabooga.apk'),
        path.join(__dirname, 'tabooga.apk'),
        path.join(__dirname, 'app_stable/tabooga.apk'),
        path.join(__dirname, '../tabooga.apk'),
        path.join(__dirname, '../app_stable/tabooga.apk')
    ];
    for (const apkPath of candidates) {
        if (fs.existsSync(apkPath)) {
            return res.download(apkPath, 'Benaa_Iraq_Tabooga.apk');
        }
    }
