...
if (!localStorage.getItem('tabooqa_free_blueprints')) {
    localStorage.setItem('tabooqa_free_blueprints', JSON.stringify(defaultBlueprintsList));
}
...
window.deleteBlueprint = function(bpId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            renderPlans();
            Swal.fire({ icon: 'success', title: 'تم الحذف بنجاح! 🎉', text: 'تمت إزالة الخريطة بالكامل ولن تظهر بعد الآن.' });
        }
    });
};
...
function renderPlans() {
    ...
    if (gallery) {
        ...
        // Dynamic Blueprints List strictly managed via adminUploads & business blueprints
        let allPlans = [...adminUploads, ...mappedBizPlans];
